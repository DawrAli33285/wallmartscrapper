...
// ==================== BROWSER POOL FOR MULTIPLE USERS ====================
const browserPool = {
  browsers: [],
  maxBrowsers: 3,  // Adjust based on your EC2 instance size (t2.medium = 2-3, t2.large = 4-5)
  
  async getBrowser() {
    // Reuse existing browser if available
    if (this.browsers.length > 0) {
      const browser = this.browsers.pop();
      console.log(`♻️ Reusing browser from pool (${this.browsers.length} remaining)`);
      return browser;
    }
  ...
  returnBrowser(browser) {
    if (this.browsers.length < this.maxBrowsers) {
      ...
      console.log(`♻️ Browser returned to pool (${this.browsers.length} available)`);
    } else {
  ...
  async closeAll() {
    console.log(`🧹 Closing all ${this.browsers.length} browsers in pool`);
...
function getChromePath() {
  ...
  if (isWindows) {
    ...
    // SNAP FIRST for Ubuntu 24.04
    possiblePaths = [
      process.env.PUPPETEER_EXECUTABLE_PATH,
      '/snap/bin/chromium',                 // Primary for Ubuntu 24.04
      '/usr/bin/chromium-browser',          
      '/usr/bin/chromium',                  
      '/usr/bin/google-chrome-stable',  
      '/usr/bin/google-chrome',        
...
async function createBrowser() {
  ...
  // Optimized for SPEED on AWS EC2
  const config = {
    headless: 'new',
    args: [
      '--no-sandbox',
...
async function loginToFacebook(page) {
  try {
    ...
    await page.type('#email', FACEBOOK_EMAIL, { delay: 80 + Math.random() * 40 });
    
    await wait(500 + Math.random() * 300);
    await page.click('#pass');
    await wait(200 + Math.random() * 200);
    await page.type('#pass', FACEBOOK_PASSWORD, { delay: 90 + Math.random() * 40 });
...
async function fetchFacebookListingDetails(facebookUrl, page, retryCount = 0) {
  ...
  try {
    ...
    // Navigate faster - don't wait for full networkidle
    await page.goto(facebookUrl, { 
      waitUntil: 'domcontentloaded',
      timeout: 30000
    });
    ...
    const listingData = await page.evaluate(() => {
      ...
      if (!data.sellerId) {
        ...
        if (profileMatches && profileMatches.length > 0) {
          ...
          if (mostCommonId && idCounts[mostCommonId] >= 2) {
            data.sellerId = mostCommonId;
            data.extractionMethods.push('HTML Source (most common profile ID)');
          }
        }
...
module.exports.enrichifyItemIds = async (req, res) => {
  ...
  try {
    ...
    // Create browser session - USE POOL
    ...
    page.on('request', (req) => {
      ...
      else if (url.includes('analytics') || url.includes('tracking') || url.includes('doubleclick') || url.includes('facebook.com/tr')) {
        req.abort();
      }
      else {
        req.continue();
    ...
    // Handle Facebook authentication - USE COOKIES FIRST
    ...
    if (cookiesLoaded) {
      ...
      console.log('🔐 No saved cookies, performing fresh login...');
      const loginSuccess = await loginToFacebook(page);
      if (!loginSuccess) {
        throw new Error('Failed to login to Facebook');
      }
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      // Adaptive delay between requests - REDUCED
    ...
    if (browser) {
      try {
        ...
        console.error('Failed to return browser:', e.message);
      }
    }
